...
export default function SalaryTOSRecon({ onBack }) {
    ...
    const updEmp = (arr, setArr, i, f, v) => setArr(prev => prev.map((e,idx) => idx===i ? {...e, [f]:v} : e));
    const addEmp = (arr, setArr) => setArr(prev => [...prev, { ...EMPTY_EMP }]);
    const remEmp = (arr, setArr, i) => setArr(prev => prev.filter((_,idx) => idx!==i));
    const updMonth = (i, f, v) => setMonthlyTOS(prev => prev.map((m,idx) => idx===i ? {...m,[f]:v} : m));
    ...
    const EmpTable = ({ label, rows, setRows, color }) => (
        <div>
            <div className="flex items-center justify-between mb-3">
                <h3 className="text-xs font-black uppercase tracking-widest" style={{color}}>{label}</h3>
                <button onClick={() => addEmp(rows, setRows)} className="flex items-center gap-1 px-3 py-1 border border-dashed rounded-lg text-[10px] font-bold transition hover:opacity-80" style={{borderColor:color, color}}>
                    ...
                    <tbody className="divide-y divide-white/5">
                        {rows.map((e,i) => (
                            <tr key={i} className="group hover:bg-white/5">
                                <td className="px-2 py-2"><input value={e.position} onChange={ev => updEmp(rows,setRows,i,'position',ev.target.value)} placeholder="Position" className="w-40 bg-slate-700 border border-slate-600 rounded px-2 py-1 text-white outline-none focus:border-blue-400 text-xs" /></td>
                                <td className="px-2 py-2"><input type="text" inputMode="decimal" value={e.count} onChange={ev => updEmp(rows,setRows,i,'count',ev.target.value)} placeholder="0" className="w-16 bg-slate-700 border border-slate-600 rounded px-2 py-1 text-white outline-none focus:border-blue-400 text-xs text-center" /></td>
                                <td className="px-2 py-2"><input type="text" inputMode="decimal" value={e.annualSalary} onChange={ev => updEmp(rows,setRows,i,'annualSalary',ev.target.value)} placeholder="0.00" className="w-28 bg-slate-700 border border-slate-600 rounded px-2 py-1 text-white outline-none focus:border-blue-400 text-xs text-right" /></td>
                                <td className="px-2 py-2"><input type="text" inputMode="decimal" value={e.fringeBenefits} onChange={ev => updEmp(rows,setRows,i,'fringeBenefits',ev.target.value)} placeholder="0.00" className="w-28 bg-slate-700 border border-slate-600 rounded px-2 py-1 text-white outline-none focus:border-blue-400 text-xs text-right" /></td>
                                <td className="px-2 py-2"><button onClick={() => remEmp(rows,setRows,i)} className="text-slate-600 hover:text-red-500 opacity-0 group-hover:opacity-100"><Trash2 size={12}/></button></td>
                            </tr>
                        ))}
                    </tbody>
                    <tfoot>
                                    ...
                                    {monthlyTOS.map((m,i) => {
                                        ...
                                        return (
                                            <tr key={i} className="hover:bg-white/5 transition">
                                                ...
                                                <td className="px-3 py-2"><input type="text" inputMode="decimal" value={m.grossSalary} onChange={e => updMonth(i,'grossSalary',e.target.value)} placeholder="0.00" className="w-28 bg-slate-700 border border-slate-600 rounded px-2 py-1 text-white outline-none focus:border-blue-400 text-xs text-right" /></td>
                                                <td className="px-3 py-2"><input type="text" inputMode="decimal" value={m.tosFiled} onChange={e => updMonth(i,'tosFiled',e.target.value)} placeholder="0.00" className="w-28 bg-slate-700 border border-slate-600 rounded px-2 py-1 text-white outline-none focus:border-blue-400 text-xs text-right" /></td>
                                                <td className="px-3 py-2"><input type="text" inputMode="decimal" value={m.tosPaid} onChange={e => updMonth(i,'tosPaid',e.target.value)} placeholder="0.00" className="w-28 bg-slate-700 border border-slate-600 rounded px-2 py-1 text-white outline-none focus:border-blue-400 text-xs text-right" /></td>
