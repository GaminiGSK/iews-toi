...
const NI_BASE = 'bg-slate-700 border border-slate-600 rounded px-2 py-1 text-white outline-none focus:border-yellow-400 text-xs';

// AssetRow MUST be outside to prevent focus-loss remount bug on keystroke
function AssetRow({ a, i, onUpdate, onRemove }) {
    const c = calcAsset(a);
    return (
        <tr className="group hover:bg-white/5 transition">
            <td className="px-2 py-2">
                <input value={a.description} onChange={e => onUpdate(i,'description',e.target.value)} placeholder="e.g. Toyota Camry"
                    className="w-36 bg-slate-700 border border-slate-600 rounded px-2 py-1 text-white outline-none focus:border-yellow-400 text-xs"/>
            </td>
            <td className="px-2 py-2">
                <select value={a.category} onChange={e => onUpdate(i,'category',e.target.value)}
                    className="bg-slate-700 border border-slate-600 rounded px-2 py-1 text-white outline-none focus:border-yellow-400 text-xs">
                    {Object.keys(GDT_RATES).map(k => <option key={k}>{k}</option>)}
                </select>
            </td>
            <td className="px-2 py-2">
                <input type="date" value={a.purchaseDate} onChange={e => onUpdate(i,'purchaseDate',e.target.value)}
                    className="bg-slate-700 border border-slate-600 rounded px-2 py-1 text-white outline-none focus:border-yellow-400 text-xs"/>
            </td>
            <td className="px-2 py-2"><NumericInput value={a.cost}          onChange={v => onUpdate(i,'cost',v)}          placeholder="0.00" className={`w-28 text-right ${NI_BASE}`}/></td>
            <td className="px-2 py-2"><NumericInput value={a.accDepOpening} onChange={v => onUpdate(i,'accDepOpening',v)} placeholder="0.00" className={`w-24 text-right ${NI_BASE}`}/></td>
            <td className="px-2 py-2"><NumericInput value={a.additions}     onChange={v => onUpdate(i,'additions',v)}     placeholder="0.00" className={`w-20 text-right ${NI_BASE}`}/></td>
            <td className="px-2 py-2"><NumericInput value={a.disposals}     onChange={v => onUpdate(i,'disposals',v)}     placeholder="0.00" className={`w-20 text-right ${NI_BASE}`}/></td>
            <td className="px-2 py-2 text-center">
                <span className="px-2 py-0.5 bg-yellow-500/10 text-yellow-400 rounded font-bold">{c.rate}%</span>
            </td>
            <td className="px-2 py-2 text-right"><span className="font-bold text-orange-400">{fmtN(c.dep)}</span></td>
            <td className="px-2 py-2 text-right"><span className={`font-bold ${c.nbv < 0 ? 'text-red-400' : 'text-green-400'}`}>{fmtN(c.nbv)}</span></td>
            <td className="px-2 py-2">
                <input value={a.invoiceRef} onChange={e => onUpdate(i,'invoiceRef',e.target.value)} placeholder="INV-001"
                    className="w-20 bg-slate-700 border border-slate-600 rounded px-2 py-1 text-white outline-none focus:border-yellow-400 text-xs"/>
            </td>
            <td className="px-2 py-2">
                <button onClick={() => onRemove(i)} className="text-slate-600 hover:text-red-500 transition opacity-0 group-hover:opacity-100"><Trash2 size={14}/></button>
            </td>
        </tr>
    );
}
...
export default function AssetDepreciation({ onBack }) {
    ...
    const handleFileUpload = (e) => {
        ...
        files.forEach(file => {
            ...
            reader.onload = (ev) => {
                setUploads(prev => [...prev, { label: file.name, fileName: file.name, mimeType: file.type, data: ev.target.result, uploadedAt: new Date().toISOString() }]);
            };
    ...
    const totals = assets.reduce((acc, a) => {
        const c = calcAsset(a);
        return { cost: acc.cost+(parseFloat(a.cost)||0), open: acc.open+(parseFloat(a.accDepOpening)||0), add: acc.add+(parseFloat(a.additions)||0), disp: acc.disp+(parseFloat(a.disposals)||0), dep: acc.dep+c.dep, nbv: acc.nbv+c.nbv };
    }, { cost:0, open:0, add:0, disp:0, dep:0, nbv:0 });

    if (loading) return <div className="min-h-screen bg-slate-900 flex items-center justify-center"><Loader2 className="animate-spin text-yellow-400" size={40}/></div>;
    ...
    return (
        <div className="min-h-screen bg-slate-900 text-white font-sans">
            <div className="bg-slate-900/90 border-b border-white/10 px-6 py-4 flex items-center gap-4 sticky top-0 z-30 backdrop-blur">
                ...
                <div>
                    <h1 className="text-lg font-black uppercase tracking-widest flex items-center gap-2"><span className="text-yellow-400">📦</span> Asset & Depreciation Register</h1>
            ...
            <div className="p-6 space-y-6 max-w-[1400px] mx-auto">
                    ...
                    <div className="grid grid-cols-3 gap-6">
                        <div>
                            ...
                            <div className="flex gap-3">
                                {['yes','no'].map(v => (
                                    <button key={v} onClick={() => setHasAssets(v)} className={`flex-1 py-2 rounded-lg text-xs font-bold border transition ${hasAssets===v ? 'bg-yellow-500 text-black border-yellow-400' : 'bg-slate-700 border-slate-600 text-slate-300 hover:border-yellow-500'}`}>
                                        {v==='yes'?'✅ Yes':'❌ No'}
                                    </button>
                                ))}
                            </div>
                        ...
                        <div>
                            ...
                            <div className="flex gap-3">
                                {['yes','no'].map(v => (
                                    <button key={v} onClick={() => setIsFirstYear(v)} className={`flex-1 py-2 rounded-lg text-xs font-bold border transition ${isFirstYear===v ? 'bg-yellow-500 text-black border-yellow-400' : 'bg-slate-700 border-slate-600 text-slate-300 hover:border-yellow-500'}`}>
                                        {v==='yes'?'Yes':'No'}
                                    </button>
                                ))}
                            </div>
                        ...
                        <div className="overflow-x-auto">
                            <table className="w-full text-xs">
                                ...
                                <tbody className="divide-y divide-white/5">
                                    {assets.map((a, i) => (
                                        <AssetRow key={i} a={a} i={i} onUpdate={updateAsset} onRemove={removeAsset} />
                                    ))}
                                </tbody>
                                <tfoot>
                ...
                <div className="bg-slate-800/60 border border-white/10 rounded-2xl p-6">
                    ...
                    <div className="mb-4 grid grid-cols-3 gap-3 text-[11px] text-slate-400">
                        {[{label:'Purchase Invoices (per asset)',hint:'PDF or image per asset'},{label:'Existing Asset Register',hint:'Excel or PDF optional'},{label:'Prior Year Tax Return',hint:'For opening pool balance'}].map((d,i) => (
