        ...
        const prompt = `
            ...
            **AVAILABLE TOOL REGISTRY**:
            ...
            8. **fill_toi_workspace**: If the user asks you to fill out the TOI form (e.g., "fill in page one"), MUST extract the entity details from the context company/profile data and output ONLY a valid JSON object in this exact format. If you cannot find certain information, you MUST use the 'reply_text' to explicitly ask the user for it.
               Schema: { "tool_use": "fill_toi_workspace", "reply_text": "I have filled in the known fields. However, I am missing [List Missing Fields]. Could you provide those?", "params": { "tin": "extract regId or taxId, ensuring hyphens if any", "name": "extract nameEn and nameKh combined", "branchOut": "001", "registrationDate": "extract incDate, e.g. 15/07/2021", "directorName": "extract director name or N/A", "businessActivities": "extract business type", "agentName": "Tax Agent Name or null", "agentLicense": "Tax Agent License Number or null", "address1": "extract EXACT Khmer address from BR docs (e.g. UNIT D414, ជាន់ទី៤...). Include the english block/unit names if they are part of the Khmer address line.", "address2": "extract exactly the same Khmer address as address1 (BOTH address1 and address2 MUST be filled with the hybrid Khmer address).", "address3": "N/A", "taxMonths": "12", "fromDate": "01012026", "untilDate": "31122026", "accountingRecord": "Using Software / Not Using Software / null", "softwareName": "Software if used", "taxComplianceStatus": "Gold / Silver / Bronze / null", "statutoryAudit": "Required / Not Required / null", "legalForm": "Private Limited Company (or match to exact English choices on form) / null", "yearFirstRevenue": "Year of first revenue or null", "yearFirstProfit": "Year of first profit or null", "priorityPeriodYear": "Priority period year or null", "incomeTaxRate": "30% / 20% / 5% / 0% / 0-20% / Progressive Rate / null", "incomeTaxDue": "Amount or null", "taxCreditCarriedForward": "Amount or null" } }
               **STRICT INTERROGATION PROTOCOL**: If ANY of the newly added tax fields (agentName, agentLicense, yearFirstRevenue, yearFirstProfit, priorityPeriodYear, incomeTaxRate, incomeTaxDue, taxCreditCarriedForward, accountingRecord, taxComplianceStatus, statutoryAudit, legalForm) are unknown or null, YOU MUST EXPLICITLY ASK THE USER TO PROVIDE THEM IN THE 'reply_text'. Your 'reply_text' MUST BE A DIRECT QUESTION to the user asking for the missing data (e.g., "What is the tax agent's name? What is your income tax rate?"). DO NOT gloss over these. Ask for them explicitly.
