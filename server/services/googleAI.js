    ...
    try {
        ...
        const prompt = `
            ...
            **AVAILABLE TOOL REGISTRY**:
            ...
            8. **fill_toi_workspace**: If the user asks you to fill out the TOI form (e.g., "fill in page one") or update a specific field on it, MUST extract the entity details from the context company/profile data and output ONLY a valid JSON object in this exact format. If you cannot find certain information, you MUST use the 'reply_text' to explicitly ask the user for it.
               Schema: { "tool_use": "fill_toi_workspace", "reply_text": "A natural conversational response acknowledging what you updated, and asking the user for exactly what you still need to complete the form.", "params": { "tin": "extract regId or taxId, ensuring hyphens if any", "name": "extract nameKh FIRST, then nameEn if Kh is missing", "branchOut": "001", "registrationDate": "extract incDate, e.g. 15/07/2021", "directorName": "extract pure KHMER director name ONLY or N/A", "businessActivities": "extract pure KHMER business type ONLY", "agentName": "Tax Agent Name or 'N/A'", "agentLicense": "Tax Agent License Number or 'N/A' (MUST BE N/A IF NONE)", "address1": "extract ONLY the pure Khmer text of the address. Strip ALL english words.", "address2": "extract exactly the same pure Khmer address as address1.", "address3": "N/A", "taxMonths": "12", "fromDate": "01012026", "untilDate": "31122026", "accountingRecord": "Using Software / Not Using Software / null", "softwareName": "Software if used", "taxComplianceStatus": "Gold / Silver / Bronze / null", "statutoryAudit": "Required / Not Required / null", "legalForm": "Private Limited Company (or match to exact English choices on form) / null", "yearFirstRevenue": "Year of first revenue or null", "yearFirstProfit": "Year of first profit or null", "priorityPeriodYear": "Priority period year or null", "incomeTaxRate": "30% / 20% / 5% / 0% / 0-20% / Progressive Rate / null", "incomeTaxDue": "Amount or null", "taxCreditCarriedForward": "Amount or null" } }
               **STRICT INTERROGATION PROTOCOL**: If ANY of the following fields are unknown or missing in the BR (agentName, agentLicense, yearFirstRevenue, yearFirstProfit, priorityPeriodYear, incomeTaxRate, incomeTaxDue, taxCreditCarriedForward, accountingRecord, taxComplianceStatus, statutoryAudit, legalForm), YOU MUST EXPLICITLY ASK THE USER TO PROVIDE THEM IN THE 'reply_text'. DO NOT hallucinate default values. Your 'reply_text' MUST BE A DIRECT QUESTION (e.g., "What is the tax agent's name? What is your income tax rate?"). Do not skip this step! Make sure to acknowledge what you DID update though!
